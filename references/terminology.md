# Terminology Guide

Use Chinese medical textbook/anatomy terms. Keep labels concise unless the original label contains an explanatory parenthesis.

## Authority

For cardiovascular labels, this file is a layout-oriented supplement, not the primary naming authority.

1. Search `translate-cardiovascular-literature-zh/references/terminology/cnterm-2025/` first.
2. A matching 2025 A0 term overrides examples in this file and older project translations.
3. Use this file for eponyms, detailed cardiac anatomy, orientation markers, and labels absent from A0.
4. Do not shorten an A0 term into a different medical concept merely to fit the image. Adjust typography or move the English explanation to the figure legend instead.

## Eponyms And Transliterations

Use Chinese standard names with English in parentheses when the term is eponymic, transliterated, or commonly recognized by English name:

- `Bachmann's bundle` -> `巴赫曼束（Bachmann's bundle）`
- `Tendon of Todaro` -> `托达罗腱（Tendon of Todaro）`
- `Eustachian valve` -> `下腔静脉瓣（Eustachian valve）`
- `Thebesian valve` -> `冠状窦瓣（Thebesian valve）`
- `Warfarin ridge` -> `华法林嵴（Warfarin ridge）`

Do not append English to ordinary anatomical labels such as `superior vena cava`, `transverse sinus`, or `left ventricle`.

## Cardiovascular Examples

- `Brachiocephalic artery` -> `头臂动脉`
- `Aortic arch` -> `主动脉弓`
- `Ascending aorta` -> `升主动脉`
- `Superior aortic recess` -> `主动脉上隐窝`
- `Inferior aortic recess` -> `主动脉下隐窝`
- `Aorto-caval recess` -> `主动脉-腔静脉隐窝`
- `Superior vena cava` -> `上腔静脉`
- `Inferior vena cava` -> `下腔静脉`
- `Transverse sinus` -> `心包横窦`
- `Right ventricular outflow tract` -> `右心室流出道`
- `Left ventricular outflow tract` -> `左心室流出道`
- `Left main trunk orifice` -> `左主干开口`
- `Left coronary aortic sinus` -> `左冠状动脉窦`
- `Right coronary aortic sinus` -> `右冠状动脉窦`
- `Non-coronary aortic sinus` -> `无冠状动脉窦`
- `Pulmonary trunk` -> `肺动脉干`
- `Right pulmonary artery` -> `右肺动脉`
- `Right superior pulmonary vein` -> `右上肺静脉`
- `Right inferior pulmonary vein` -> `右下肺静脉`
- `Left common pulmonary vein` -> `左共同肺静脉`
- `Coronary sinus` -> `冠状窦`
- `Coronary sinus orifice` -> `冠状窦口`
- `Atrioventricular nodal artery` -> `房室结动脉`
- `Left circumflex artery` -> `左回旋支`

## Cardiac Anatomy Examples

- `Right ventricle` -> `右心室`
- `Left ventricle` -> `左心室`
- `Right atrial appendage` -> `右心耳`
- `Left atrial appendage` -> `左心耳`
- `Fossa ovalis` -> `卵圆窝`
- `Crista terminalis` -> `界嵴`
- `Sinus venarum` -> `静脉窦部`
- `Terminal groove` -> `界沟`
- `Anterior interventricular groove` -> `前室间沟`
- `Right atrioventricular groove` -> `右房室沟`
- `Left atrioventricular groove` -> `左房室沟`
- `Acute margin` -> `锐缘`
- `Ventricular septum` -> `室间隔`
- `Membranous septum` -> `膜性间隔`
- `Central fibrous body` -> `中央纤维体`
- `Right fibrous trigone` -> `右纤维三角`
- `Left fibrous trigone` -> `左纤维三角`
- `Inferior pyramidal space` -> `下锥体间隙`
- `Interleaflet triangle` -> `瓣间三角`
- `Moderator band` -> `节制束`
- `Right bundle branch` -> `右束支`

## Valve And Muscle Examples

- `Tricuspid leaflet` -> `三尖瓣瓣叶`
- `Septal tricuspid leaflet` -> `三尖瓣隔瓣`
- `Inferior tricuspid leaflet` -> `三尖瓣下瓣叶`
- `Anterior mitral leaflet` -> `二尖瓣前瓣`
- `Posterior mitral leaflet` -> `二尖瓣后瓣`
- `Papillary muscle` -> `乳头肌`
- `Anterior papillary muscle` -> `前乳头肌`
- `Medial papillary muscle` -> `内侧乳头肌`
- `Inferomedial papillary muscle` -> `下内侧乳头肌`
- `Superolateral papillary muscle` -> `上外侧乳头肌`
- `Right adjacent pulmonary leaflet` -> `右邻接肺动脉瓣叶`
- `Left adjacent pulmonary leaflet` -> `左邻接肺动脉瓣叶`
- `Non-adjacent pulmonary leaflet` -> `非邻接肺动脉瓣叶`

## Direction Marker

Translate the yellow orientation marker compactly:

- `Superior` -> `上`
- `Inferior` -> `下`
- `Right posterior` -> `右后`
- `Left anterior` -> `左前`

Keep the cross marker and yellow color if present.
